...
import type { Visit, VisitFormData, TimestampField, Enquiry } from '../../lib/types';
...
export default function AdminPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChange((user) => {
      // Debug log to help diagnose why header/buttons may not show
      console.debug('onAuthStateChange -> user:', user?.email, user);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot: QuerySnapshot<DocumentData>) => {
      ...
      console.error('Firestore error for enquiries:', error);
    });
          ...
          <div className="w-full bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            <div className="text-center">
              ...
              <button
                ...
                className="w-full flex items-center justify-center px-6 py-4 border border-gray-200 rounded-xl shadow-sm bg-white text-base font-medium text-gray-700 hover:bg-gray-50 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 mb-4"
              >
              ...
              {/* Temporary debug info: shows current user/email detected by app */}
              <div className="mt-4 text-xs text-red-600">
                DEBUG: current user email = {String((user as any)?.email || 'null (not signed in)')}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link 
                ...
                className="bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2 rounded-xl text-sm transition-colors duration-200 flex items-center space-x-2"
              >
